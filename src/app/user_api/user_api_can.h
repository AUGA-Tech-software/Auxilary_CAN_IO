...
uint8_t user_can_db_transmit_stop_bus(const uint8_t can_bus, const uint8_t status);
#define user_can_db_stop_transmission(can_bus, status) user_can_db_transmit_stop_bus(can_bus, status)

void user_can_db_transmit_stop_block( const uint32_t block_id, const uint8_t status );
#define user_can_db_set_transmit_stop(block_id, status) user_can_db_transmit_stop_block(block_id, status)

uint8_t user_can_db_transmit_stop( const uint8_t status );
#define user_can_db_transmit_deactivate(status) user_can_db_transmit_stop(status)
