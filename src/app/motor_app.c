...
static void motor_filter_current( uint8_t ch, uint32_t* ptr_saved, uint32_t* ptr_filtered, uint32_t* ptr_ofst );
static uint32_t get_curr( uint8_t channel, uint32_t offset );
...
enum_motor_err motor_set_hb_type(uint8_t motor, enum_motor_bridge_mode hb_mode)
{
...
enum_motor_err motor_set_direction(uint8_t motor, enum_motor_dir dir)
{
    enum_motor_err err = MOTOR_ERR_GENERAL;
...
enum_motor_err motor_ramp_up(uint8_t motor, uint16_t target_duty, uint16_t up_speed_ms)
{
    enum_motor_err err = MOTOR_ERR_GENERAL;
...
enum_motor_err motor_ramp_down(uint8_t motor, uint16_t target_duty, uint16_t down_speed_ms)
{
    enum_motor_err err = MOTOR_ERR_GENERAL;
...
enum_motor_err motor_set_frequency(uint8_t motor, uint16_t frequency)
{
    enum_motor_err err = MOTOR_ERR_GENERAL;
...
enum_motor_err motor_set_duty_cycle(uint8_t motor, uint16_t duty_cycle)
{
    enum_motor_err err = MOTOR_ERR_GENERAL;
...
enum_motor_err motor_update(uint8_t motor, uint32_t *curr)
{
...
enum_motor_err motor_cyclic(uint8_t motor, uint16_t pwm_freq, uint16_t pwm_duty, uint16_t ramp_max, uint16_t ramp_up, uint16_t ramp_dwn, uint8_t dir, uint8_t motor_check, uint32_t* curr)
{
    enum_motor_err err = MOTOR_ERR_GENERAL;
    ...
    switch( handle->state )
    {
        case STATE_INIT:
        {
            ...
            if( user_time_past( handle->timer, STARTUP_TIMER, PRECISION_1MS ) )
            {
                if (motor_curr_p) 
                {
                    motor_curr_p->offset = user_ai_get_mv_calib( curr_p_pin );
                }

                if (motor_curr_n) 
                {
                   motor_curr_n->offset = user_ai_get_mv_calib( curr_n_pin );
                }
        ...
        case STATE_RUN:
        {
            if (motor_curr_p) 
            {
                motor_filter_current( curr_p_pin, &motor_curr_p->saved, &motor_curr_p->filtered, &motor_curr_p->offset );
            }

            if (motor_curr_n) 
            {
                motor_filter_current( curr_n_pin, &motor_curr_n->saved, &motor_curr_n->filtered, &motor_curr_n->offset );
            }
            ...
            else
            {
            	if(handle->config->direction != handle->dir_saved)
            	{
            		if( handle->ramp_control.ramp_value_end != 0 )
            		{
            			handle->ramp_control.ramping = 1;
                		handle->ramp_control.ramp_value_start = (int32_t)handle->duty_out;
                		handle->ramp_control.ramp_value_end = 0;
                		handle->ramp_control.ramp_time = (int32_t)handle->config->ramp_dwn;
    					user_timer_set_timestamp(&handle->ramp_control.ramp_time_start, HAL_PRECISION_1MS);
            		}


					uint32_t time_elapsed;
					user_timer_set_timestamp(&time_elapsed, HAL_PRECISION_1MS);
					time_elapsed -= handle->ramp_control.ramp_time_start;

					handle->duty_out = sfl_ramp_generator(&handle->ramp_control, time_elapsed);
				...
				{
            		if( (handle->ramp_control.ramp_time != handle->config->ramp_up) || (handle->ramp_control.ramp_value_end != handle->config->pwm_duty))
            		{
            			handle->ramp_control.ramping = 1;
						handle->ramp_control.ramp_value_start = (int32_t)handle->duty_out;
						handle->ramp_control.ramp_value_end = (int32_t)handle->config->pwm_duty;
						handle->ramp_control.ramp_time = (int32_t)handle->config->ramp_up;
						user_timer_set_timestamp(&handle->ramp_control.ramp_time_start, HAL_PRECISION_1MS);
            		}
					...
					if(handle->duty_out == handle->config->pwm_duty)
					{
						time_elapsed = 0;
					}
				}
...
static void motor_filter_current( uint8_t ch, uint32_t* ptr_saved, uint32_t* ptr_filtered, uint32_t* ptr_ofst )
{
        *ptr_filtered = get_curr(ch, *ptr_ofst );
}
