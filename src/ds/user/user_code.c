...
uint8_t cmd_datapoints[14] = 	{CMD_C3, 		CMD_C2, 	CMD_C1, 	CMD_B1, 	CMD_B8, 	CMD_C8, 	CMD_C7, 	CMD_C6,		CMD_B6, 	CMD_A6, 	CMD_A5, 	CMD_C4, 	CMD_A4,		CMD_A3};		// CAN datapoints of MRS Output commands
uint8_t status_datapoints[14] = {STATUS_C3, 	STATUS_C2, 	STATUS_C1, 	STATUS_B1, 	STATUS_B8, 	STATUS_C8, 	STATUS_C7, 	STATUS_C6,	STATUS_B6, 	STATUS_A6, 	STATUS_A5, 	STATUS_C4, 	STATUS_A4,	STATUS_A3};		// CAN datapoints of MRS Input states
uint8_t cur_datapoints[8] = 	{CUR_C3, 		CUR_C2, 	CUR_C1, 	CUR_B1, 	CUR_B8, 	CUR_C8, 	CUR_C7, 	CUR_C6};		// CAN datapoints of MRS Input currents
uint8_t out_pins[14] = 			{OUT_PIN_C3,	OUT_PIN_C2, OUT_PIN_C1, OUT_PIN_B1, OUT_PIN_B8, OUT_PIN_C8, OUT_PIN_C7, OUT_PIN_C6,	OUT_PIN_B6, OUT_PIN_A6,	OUT_PIN_A5,	OUT_PIN_C4,	OUT_PIN_A4,	OUT_PIN_A3};	// MRS Output pins
uint8_t in_pins[14] = 			{IN_PIN_C3, 	IN_PIN_C2, 	IN_PIN_C1, 	IN_PIN_B1, 	IN_PIN_B8, 	IN_PIN_C8, 	IN_PIN_C7, 	IN_PIN_C6,	IN_PIN_B6, 	IN_PIN_A6, 	IN_PIN_A5, 	IN_PIN_C4, 	IN_PIN_A4,	IN_PIN_A3};		// MRS Input pins
uint8_t ain_pins[8] = 			{AIN_PIN_C3,	AIN_PIN_C2, AIN_PIN_C1, AIN_PIN_B1, AIN_PIN_B8, AIN_PIN_C8, AIN_PIN_C7, AIN_PIN_C6};	// MRS Input pins with current sense

uint32_t previousMillis = 0;
uint32_t currentMillis;

uint8_t total_current = 0;

uint8_t my_sci_buffer[3];


// Returns timestamp_now
uint32_t millis() {
	uint32_t timestamp_now;
	user_timer_set_timestamp(&timestamp_now, PRECISION_1MS);
...
void usercode_init(void)
{
	...
	for (uint8_t q = 0; q < 14; q++) {
		modulhardwarecode_adc_set_measurement_range(in_pins[q], AI_RANGE_16V);
		// Sending PIN statuses over CAN
	}
...
void usercode(void)
{
	...
	user_can_db_process_rx();
	// Check CAN for commands, and set pins accordingly
	for (uint8_t i = 0; i < 14; i++) {
		// Receive every PIN command from CAN
		if (user_can_db_get_signal_value(cmd_datapoints[i]) == 1) {
			// set digital output pin
			user_io_set_do(out_pins[i], true);
		} else {
			user_io_set_do(out_pins[i], false);
		}
	}
	
	// Check states of output pins
	for (uint8_t j = 0; j < 14; j++) {
		// read state of digital output pin
		uint8_t readback_of_pin = user_io_readback_do(out_pins[j]);
		// Sending PIN statuses over CAN
		user_can_db_set_signal_value(status_datapoints[j], readback_of_pin);
	}

	// Check current of pins and send it to CAN
	for (uint8_t k = 0; k < 8; k++) {
		// get state of digital input pin
		uint32_t current_of_pin = user_ai_get_mv_calib(ain_pins[k]);
		// If current (mA) is higher than 2.5A - cap it to 2.5A
		if(current_of_pin >= 2550) {
			current_of_pin = 2550;
		}
		uint8_t scaled_current = current_of_pin / 10;
		total_current += scaled_current;
		// Sending PIN statuses over CAN
		user_can_db_set_signal_value(cur_datapoints[k], scaled_current);
	}
	


	
	// MEasure current of pin C1 (PPP) DO_HSD1_OUT2
	// uint32_t current_of_pin = user_ai_get_mv_calib(ain_pins[2]);
	user_can_db_set_signal_value(STATUS_TEST, total_current);
	total_current = 0;


	

	// // PERIODIC TESTING GROUND
	// currentMillis = millis();

	// // Here goes test code that needs to be fired, every 3 sec.
...
//---------------------------------------------------------------------------------------------------------------------
//	Procedure		uint16_t user_ai_get_mv_calib(enum_adc_pin_name pin);
//
//	Overview		get the scaled value in mV/mA of the analog input for non-calibrated inputs or calibrated value for calibrated inputs
//
//	Input			pin			                 - enum_adc_pin_name 		inside io_tables.h
//  Output          uint32_t                     - Value of the analog input in mV or calibrated value for calibrated channels
                                                   input pin

	Example:		uint32_t get_in0_value;
	                get_in0_value = user_ai_get_mv_calib(AI_A_IN0);
...
//  Output          enum_HAL_CAN_RETURN_VALUE   - Return code, 0 = success
    ...
    Example:        uint8_t msg_buffer[] = {0xAA, 0xBB, 0xCC};
                    user_can_send_msg_buffer(CAN_BUS_0, 0x123, STANDARD_ID, sizeof(msg_buffer), msg_buffer);
...
//---------------------------------------------------------------------------------------------------------------------
//  Procedure       uint8_t user_can_db_transmit_stop_bus(const uint8_t can_bus, const uint8_t status);
//
...
//                                                              1 = error
    ...
    Example:        user_can_db_transmit_stop_bus(0,1);

//---------------------------------------------------------------------------------------------------------------------
//  Procedure       uint8_t user_can_db_transmit_stop_block( const uint32_t id, const uint8_t status );
//
...
//                  status                       - const uint8_t     	status = 1 -> disable sending of CAN message with specified block ID
    ...
    Example:        user_can_db_transmit_stop_block(CanTelegramm_0, 1);

//---------------------------------------------------------------------------------------------------------------------
// Procedure        uint8_t user_can_db_transmit_stop(const uint8_t status);
//
...
//                                                              1 = error
    ...
    Example:        user_can_db_transmit_stop(1);
...
//  Output          uint32_t                     - PI controller calculated duty cycle.
    ...
    Example:        user_pwm_current_control(PWM_HSD1_OUT0, 500, 400, 20);  // Example for a 12V valve with 5.2 ohm coil resistance
...
// Procedure        uint32_t user_motor_control(uint8_t motor, uint16_t pwm_freq, uint16_t duty, uint16_t ramp_max, uint16_t ramp_up, uint16_t ramp_dwn, enum_motor_dir dir, uint8_t motor_check, uint32_t* curr);
//
//
//
...
//
// Input            motor                        - uint8_t   			Motor for which the cyclic control is executed.
...
// Output           uint32_t                     - Return code,         0 = MOTOR_ERR_OK,                   // Operation successful
                                                                        ...
                                                                        Note: If an error occurs (e.g. overcurrent error), the error recovery procedure involves
                                                                        resetting the duty cycle to 0. After that, the motor controller can be properly used again.
