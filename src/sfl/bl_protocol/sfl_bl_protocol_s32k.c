...
enum_SFL_BLP_ERROR_CODES sfl_bl_protocol_s32k_set_baudrate( const struct_hal_can_handle* ptr_can_handle, uint8_t baudrate )
{
    ...
    if( mgl_bl_can_ind == ptr_can_handle->can_handle_number )
    {
		...
		struct_EE_CAN_EXT *ptr_can_ext = ((struct_EE_CAN_EXT*)EE_READ_PTR(can_ext_bt));
		struct_EE_CAN_EXT ext_baud = *ptr_can_ext;
		if(ext_baud.enable == true)
		{
			ext_baud.enable = false; 	
			struct_CRC8 crc_info = {.ptr_data = (uint8_t*)&ext_baud, .length = (sizeof(struct_EE_CAN_EXT) - 1u), .crc = 0u};
			(void)role_do(ROLE_ID_PROTOCOL_BOOTLOADER, ROLE_BL_PROTOCOL_CALC_CRC8, &crc_info);
			ext_baud.crc8 = (uint8_t)crc_info.crc;
			EE_WRITE(can_ext_bt, (const uint8_t*)&ext_baud);
    	}
